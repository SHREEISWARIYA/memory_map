...
ApplicationWindow {
    ...
    Connections {
        target: pastTrail
        function onTrackHistoryFetched(uuid, trackHistory) {
            handleTrackHistoryFetched(uuid, trackHistory)
        }
    }

    // Modify the handleTrackHistoryFetched function
    function handleTrackHistoryFetched(uuid, trackHistory) {
        ...
        // Add coordinates to the path and create points
        for (var i = 0; i < trackHistory.length; i++) {
            var point = trackHistory[i]
            var coordinate = QtPositioning.coordinate(point.latitude,
                                                      point.longitude)
            path.push(coordinate)

            // Create and add the point marker
        ...
        if (trackHistory.length > 0) {
            mapview.center = QtPositioning.coordinate(trackHistory[0].latitude,
                                                      trackHistory[0].longitude)
            mapview.zoomLevel = 8
        }

        console.log("Track history displayed with", path.length, "points")
    }
    Component {
    ...
    function hidePastTrack() {
        // Remove all track history lines, points, and ship markers
        for (var i = mapview.mapItems.length - 1; i >= 0; i--) {
            var item = mapview.mapItems[i]
            if (item.objectName === "trackHistoryLine"
                    || item.objectName === "trackHistoryPoint"
                    || (item.objectName
                        && (item.objectName.startsWith("shipMarker_")
                            || item.objectName.startsWith("shipLabel_")))) {
                mapview.removeMapItem(item)
            }
        }

        // Hide the ship label and marker item if they exist
        if (typeof shipLabel !== 'undefined')
            shipLabel.visible = false
        if (typeof markerItem !== 'undefined')
            markerItem.visible = false

        console.log("Past track, points, and ship markers hidden")
    }

    // Add a function to show the past track
    function showPastTrack(uuid) {
        // Request to fetch and display the track history again
        pastTrail.fetchTrackHistory(uuid, 24) // Assuming 24 hours of history

        // Show the ship label and marker item if they exist
        if (typeof shipLabel !== 'undefined')
            shipLabel.visible = true
        if (typeof markerItem !== 'undefined')
            markerItem.visible = true

        console.log("Requested to show past track for UUID:", uuid)
    }
    ...
    function markShipOnMap(name, latitude, longitude) {
        ...
        console.log("Map centered at", mapview.center.latitude,
                    mapview.center.longitude, "with zoom level",
                    mapview.zoomLevel)
    ...
    Component.onCompleted: {
        ...
        console.log("Main QML completed. pastHistory is null:",
                    pastHistory === null)
    }
