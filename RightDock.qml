...
Drawer {
    ...
    Column {
        ...
        Text {
            anchors.horizontalCenter: parent.horizontalCenter
            text: "Total Valid Ships: " + Object.keys(shipData.ships).filter(
                      key => isValidShip(shipData.ships[key])).length
            font.pixelSize: 16
        }
        ...
        Text {
            ...
            text: {
                if (!infoPanel.currentShip)
                    return "Click on a ship to see details"
                ...
                let speed = ship.speed_over_ground
                    !== undefined ? `Speed: ${ship.speed_over_ground} knots` : "Speed: N/A"
                let course = ship.course_over_ground
                    !== undefined ? `Course: ${ship.course_over_ground}°` : "Course: N/A"
                let uuid = ship.uuid
                return `${vesselName}\n${mmsi}\n${imo}\n${lat}\n${lon}\n${speed}\n${course}\n${uuid}`
            }
            font.pixelSize: 12
        ...
        Button {
            ...
            onClicked: {
                console.log("Requesting track history for UUID:",
                            currentShipUuid)
                if (currentShipUuid) {
                    ...
                    infoPanel.shipDetailsRequested(currentShip.track_name,
                                                   currentShip.latitude,
                                                   currentShip.longitude)
                } else {
